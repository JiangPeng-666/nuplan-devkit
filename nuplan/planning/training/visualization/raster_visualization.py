...
def get_raster_with_trajectories_as_rgb_from_rgb(
        pixel_size: float,
    ...
    image = raster
    if target_trajectory is not None:
        _draw_trajectory_255(image, target_trajectory, target_trajectory_color, pixel_size)

    if predicted_trajectory is not None:
        _draw_trajectory_255(image, predicted_trajectory, predicted_trajectory_color, pixel_size)

    return image


def _draw_trajectory_255(image: npt.NDArray[np.float32], trajectory: Trajectory, color, pixel_size: float) -> None:
    """
    Draws a trajectory overlayed on an RGB image.
    ...
    image[(pixels_x[idxs], pixels_y[idxs])] = color